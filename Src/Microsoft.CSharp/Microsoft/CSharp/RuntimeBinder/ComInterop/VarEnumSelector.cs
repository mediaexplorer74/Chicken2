...
// Type: Microsoft.CSharp.RuntimeBinder.ComInterop.VarEnumSelector
...
namespace Microsoft.CSharp.RuntimeBinder.ComInterop
{
  ...
  {
    private static readonly Dictionary<VarEnum, Type> s_comToManagedPrimitiveTypes = VarEnumSelector.CreateComToManagedPrimitiveTypes();
    private static readonly IList<IList<VarEnum>> s_comPrimitiveTypeFamilies = VarEnumSelector.CreateComPrimitiveTypeFamilies();

    [RequiresUnreferencedCode("Using dynamic types might cause types or members to be removed by trimmer.")]
    internal VarEnumSelector(Type[] explicitArgTypes)
    {
      this.VariantBuilders = new VariantBuilder[explicitArgTypes.Length];
      for (int index = 0; index < explicitArgTypes.Length; ++index)
        this.VariantBuilders[index] = this.GetVariantBuilder(explicitArgTypes[index]);
    }
    ...
    internal static Type GetManagedMarshalType(VarEnum varEnum)
    {
      ...
      if (Variant.IsPrimitiveType(varEnum))
        return VarEnumSelector.s_comToManagedPrimitiveTypes[varEnum];
      switch (varEnum)
      {
    ...
    private static List<VarEnum> GetConversionsToComPrimitiveTypeFamilies(Type argumentType)
    {
      ...
      foreach (IEnumerable<VarEnum> primitiveTypeFamily in (IEnumerable<IList<VarEnum>>) VarEnumSelector.s_comPrimitiveTypeFamilies)
      {
        foreach (VarEnum key in primitiveTypeFamily)
        {
          Type managedPrimitiveType = VarEnumSelector.s_comToManagedPrimitiveTypes[key];
    ...
    private static void CheckForAmbiguousMatch(Type argumentType, List<VarEnum> compatibleComTypes)
    {
      if (compatibleComTypes.Count > 1)
      {
        ...
        {
          string name = VarEnumSelector.s_comToManagedPrimitiveTypes[compatibleComTypes[index]].Name;
